#!/usr/bin/env python3
r"""Part 3 target dump v11: v10 + verified tamed detection.

Tamed rules (in priority order):
  1. byte at tamed.boolOffset (0x14c0, bIsTamed) != 0
  2. int32 at tamed.teamIDOffset (0x148c, TamingTeamID) not in wildTeamValues
  3. backup byte at tamed.backupBoolOffset (0x14d0) != 0
  4. fallback: owner PlayerState/Controller pointer found in actor blob

Usage:
python scripts/ark_part3_target_dump_v11.py --radius 100000 --out work/targets_v11.json
"""
...
def read_level(handle, a, offsets, ranges):
    A = offsets.get("actor", {})
    CH = A.get("character", {})
    status_off = pint(CH.get("statusComponentOffset",
                             A.get("statusComponentOffset", "0xd10")))
    level_off = pint(CH.get("levelOffset", A.get("levelOffset", "0x6cc")))
...
def read_tamed(handle, a, offsets):
    """Verified tamed rules from tamed_probe:
       bIsTamed byte @0x14c0, TamingTeamID @0x148c, backup byte @0x14d0."""
    A = offsets.get("actor", {})
    T = A.get("tamed", {})
    wild_teams = T.get("wildTeamValues", [0, -1])

    bool_off = pint(T.get("boolOffset", "0x14c0"))
    if bool_off is not None:
        raw = p2.read_memory(handle, a + bool_off, 1)
        if raw and raw[0] != 0:
            return True

    team_off = pint(T.get("teamIDOffset", "0x148c"))
    if team_off is not None:
        tv = p2.read_i32(handle, a + team_off)
        if tv is not None and tv not in wild_teams:
            return True

    backup_off = pint(T.get("backupBoolOffset", "0x14d0"))
    if backup_off is not None:
        raw = p2.read_memory(handle, a + backup_off, 1)
        if raw and raw[0] != 0:
            return True
...
def main():
    ...
    ap.add_argument("--out", default="work/targets_v11.json")
    ...
    if not controller:
        (
            controller, local_ps, own,
            structural_ps_off, structural_pawn_off,
    ...
    def get_ps_name(ps):
        ...
        if best is None and not args.no_scan_player_names:
            ...
            for c in scan_fstrings(handle, ps, lo, hi):
                s = c.get("value")
                if not s:
                    continue
                sc = score_name(s, args.player_name)
    ...
    for a in creatures:
        ...
        if not is_player:
            # v11 verified tamed rules first (cheap direct reads)
            if read_tamed(handle, a, offsets):
                label = "tamed"
            else:
                # fallback: owner pointer anywhere in blob
