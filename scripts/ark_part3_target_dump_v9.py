#!/usr/bin/env python3
r"""Part 3 target dump v9: repaired filtering, dynamic PlayerState name resolution.

Repairs compared to v8:
- filters zero-location actors by default
- filters dead actors by default
- filters invalid half-height actors by default
- supports actor.playerState.nameOffsetCandidates
- localPlayerControllerOffset is optional, not a hard missing field
- better fallback name selection

Usage:
python scripts/ark_part3_target_dump_v9.py --radius 1000000 --out work/targets_v9_large.json

Debug/unfiltered:
python scripts/ark_part3_target_dump_v9.py --radius 1000000 --include-dead --include-zero --include-invalid --out work/targets_v9_debug.json
"""
...
def pint(v):
    """Parse int from offset value: int or hex string."""
    if v is None:
        return None
    if isinstance(v, int):
        return v
...
def read_blob_chunked(handle, addr, size, chunk=0x1000):
    """Read memory in chunks because large single reads can fail."""
    out = bytearray()
    off = 0
...
def read_fstring(handle, obj, off, max_len=256):
    """Read UTF-16 FString field at obj+off."""
...
def score_name(s, preferred=None):
    """Score PlayerState name candidates without hardcoding the final name."""
    ...
    # Avoid generic strings, but still allow them if nothing better exists.
...
def get_loc(handle, a, offsets, ranges, allow_zero=False):
    ...
    Primary: pawn direct location offset.
    Fallback: capsule component location, then root component location.
    """
    A = offsets.get("actor", {})
    CH = A.get("character", {})
...
def find_local_controller(
    ...
    """Try to find local controller, PlayerState, and pawn."""
    A = offsets.get("actor", {})
    C = A.get("controller", {})
    ...
    # 1. LocalPlayer path if possible.
    lp = None
    try:
        engine = p2.read_u64(
            handle,
    ...
    # 2. Known controller member offsets on controller candidates.
...
def find_structural_controller(handle, actors, pstates, creatures):
    """Fallback structural discovery: actor blob contains PlayerState + pawn."""
    pset = set(pstates)
    cset = set(creatures)
            ...
            for pawn in creatures:
                ...
                if i2 >= 0 and i2 % 8 == 0:
                    return (
                        ...
                        ps,
                        pawn,
                        f"structural PlayerState +{i1:#x}",
                        f"structural pawn +{i2:#x}",
                    )
...
def best_actor_name(handle, idmap, a, ch, kind):
    """Best fallback name for non-player actors.

    Prefer resolved object name if it is not an unresolved id.
    Then prefer first resolved class-chain name.
    Then fall back to object name/id.
    """
...
def main():
    ...
    ap.add_argument("--out", default="work/targets_v9.json")
    ...
    # Build pawn -> PlayerState map from known controller offsets.
    pawn_to_ps = {}
    ...
    for a in creatures:
        ...
        # Tamed/wild ownership check for non-player creatures.
        if not is_player:
    ...
    if (
        pint(PS.get("nameOffset")) is None
        and not PS.get("nameOffsetCandidates")
    ):
        missing.append("actor.playerState.nameOffset or nameOffsetCandidates")
